...
\[
    ...
    \marginnote{
        \footnotemark}
    \]
\footnotetext{в левой части, фактически, стоит \( \ad{A}{\Gamma X} \)
...
\begin{lemma}
    Пусть \( \norm{\cdot} \) --- суб-мультипликативная норма в \( \matr{N}{} \),
    \[ \beta = \norm{B}, \]
    \[ \gamma = \sup_{\norm{X}=1} \norm{\Gamma X}. \]
    Если
    \[ \gamma\beta \leq \frac14. \]
    ...
    Если, дополнительно, норма \( \norm{\cdot} \) согласована с нормой в \( \mathscr{X} \),
    то для всех \( { \lambda\in\spec{A-B}\setminus\{\lambda_1\} } \) имеет место
    \[ \lvert\lambda\rvert \leq r_0. \]
\end{lemma}

Теперь требуемые результаты мы получим оценив \( \beta, \gamma \).

Рассмотрим в \( \matr{N}{} \) операторную норму~
...
(обозначим её \( \operatornorm{\cdot} \))
--- её согласованность с нормой \( \mathscr{X} \)
позволит нам дать верхнюю оценку и для \( \spr{-X_{22}^o} \).
...
\begin{thm}[О грубой оценке собственных значений возмущённой матрицы единиц]
    ...
    \[ \lvert \lambda_1 - N \rvert \leq 4\sqrt{M} \leq N ,\]
    и для всех \( \lambda\in\spec{\almostallones{M}{N}} \setminus \{\lambda_1\} \)
